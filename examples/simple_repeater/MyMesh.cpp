...
// Provide the global symbol declared as `extern AbstractBridge* bridge;` in
// `MyMesh.h`. Some code includes that header and may expect a global pointer
// to the active bridge. Initialize to nullptr; will be set in MyMesh::begin().
AbstractBridge* bridge = nullptr;

#if defined(WITH_MQTT_BRIDGE)
extern MQTTBridge *g_instance;
#endif
...
void MyMesh::injectReceivedRaw(const uint8_t* raw, int len, float snr, float rssi) {
  // Simulate the behaviour of Dispatcher::checkRecv() -> processRecvPacket
  mesh::Packet* pkt = obtainNewPacket();
  if (pkt == NULL) {
    MESH_DEBUG_PRINTLN("%s injectReceivedRaw(): no free packets\n", getLogDateTime());
    return;
  }

  // restore packet from raw bytes (wire format)
  if (!pkt->readFrom(raw, (uint8_t)len)) {
    MESH_DEBUG_PRINTLN("%s injectReceivedRaw(): failed to parse raw packet\n", getLogDateTime());
    releasePacket(pkt);
    return;
  }

  // assign received SNR
  pkt->_snr = (int8_t)(snr * 4);

  // compute score and airtime similar to radio path
  float score = _radio->packetScore(snr, len);
  uint32_t air_time = _radio->getEstAirtimeFor(len);

  // Decide flood vs direct handling
  if (pkt->isRouteFlood()) {
    ...
    if (_delay < 50) {
      ...
      if (action == ACTION_RELEASE) {
        ...
        // subclass will hold packet
      } else {
        uint8_t pri = (action >> 24) - 1;
        uint32_t delay = action & 0xFFFFFF;
        sendPacket(pkt, pri, delay);
      }
    } else {
      if (_delay > 32000) _delay = 32000;
      // schedule inbound handling via queue: use same semantics as queueInbound
      _mgr->queueInbound(pkt, futureMillis(_delay));
    }
  } else {
    // direct route -> process immediately
    mesh::DispatcherAction action = onRecvPacket(pkt);
    if (action == ACTION_RELEASE) {
      releasePacket(pkt);
    } else if (action == ACTION_MANUAL_HOLD) {
...
void MyMesh::logRx(mesh::Packet *pkt, int len, float score) {
  ...
  {
    // Dump received packet bytes as hex for easier debugging
    uint8_t tmp_buf[512];
    int blen = pkt->writeTo(tmp_buf);
    char hexbuf[1024];
    mesh::Utils::toHex(hexbuf, tmp_buf, blen);
    BRIDGE_DEBUG_PRINTLN("Reception RX data=%s\n", hexbuf);

  #if defined(WITH_MQTT_BRIDGE)
    mqttBridge.publishRxHex(tmp_buf, blen);
  #endif
  }
#endif
...
#if BRIDGE_DEBUG && ARDUINO
{
  if (!_initialized || !_client.connected() || !pkt) {
    return;
  }
  uint8_t tmp_buf[512];
  int blen = pkt->writeTo(tmp_buf);
  ...
  BRIDGE_DEBUG_PRINTLN(
    "MQTT FORCE publish RX HEX len=%d ok=%d topic=%s\n",
    strlen(hexbuf), ok, rxTopic.c_str()
  );
}
#endif*/
...
void MyMesh::begin(FILESYSTEM *fs) {
  ...
  // Expose the instance bridge via the global pointer for compatibility
  // with existing code that references `bridge` as an extern symbol.
  ::bridge = &this->bridge;
