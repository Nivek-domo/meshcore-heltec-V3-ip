      ...
      if (_rx_buffer_pos >= 4) {
        ...
        if (_rx_buffer_pos == len + SERIAL_OVERHEAD) { // Full packet received
          ...
          if (validateChecksum(_rx_buffer + 4, len, received_checksum)) {
            ...
            // dump received payload as hex for debug
            const size_t rx_hex_len = len * 2 + 1;
            char *rx_hex = (char *)malloc(rx_hex_len);
            if (rx_hex) {
              mesh::Utils::toHex(rx_hex, _rx_buffer + 4, len);
              BRIDGE_DEBUG_PRINTLN("Recep data=%s\n", rx_hex);
              free(rx_hex);
            }
...
void RS232Bridge::sendPacket(mesh::Packet *packet) {
  ...
  if (!_seen_packets.hasSeen(packet)) {
    ...
    // dump transmitted payload as hex for debug
    const size_t tx_hex_len = len * 2 + 1;
    char *tx_hex = (char *)malloc(tx_hex_len);
    if (tx_hex) {
      mesh::Utils::toHex(tx_hex, buffer + 4, len);
      BRIDGE_DEBUG_PRINTLN("Transmission data=%s\n", tx_hex);
      extern MQTTBridge *g_instance;  // déjà présent dans MQTTBridge.cpp

      if (g_instance) {
        g_instance->sendPacketForce(packet);
      }
