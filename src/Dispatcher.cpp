...
#if MESH_PACKET_LOGGING
  // Debug: log action returned by onRecvPacket for easier tracing of injected packets
  {
    uint8_t pri = (action >> 24) - 1;
    uint32_t delay = action & 0xFFFFFF;
    Serial.print(getLogDateTime());
    Serial.printf(": processRecvPacket action=0x%08X pri=%d delay=%d header=%02X type=%d\n", (uint32_t)action, pri, delay, pkt->header, pkt->getPayloadType());
  }
#endif
